  ...
  def after_save(transaction)
    ...
    when 'charge'
      if transaction.success?
        SubscriptionMailer.deliver_charge_success(sub, transaction)
      else 
        sub.increment!(:warning_level)
        case sub.warning_level
        when 1
          SubscriptionMailer.deliver_charge_failure(sub, transaction)
        when 2
          SubscriptionMailer.deliver_second_charge_failure(sub, transaction)
        else
    ...
    when 'credit', 'refund'
      if transaction.success?
        SubscriptionMailer.deliver_credit_success(sub, transaction)
      end
      # else no email
